...
import SitesTransactions from "@/components/Sites/exsting/SitesReport";
import {
  Grid,
  Button,
  Typography,
...
function ExistingSitesReport(props) {
  ...
  // Fetch all existing sites data (site_type = 2)
  const fetchAllExistingSites = useCallback(async () => {
    setLoading(true);
    setError("");
    try {
      const reqData = {
        ...
        searchTerm: "",
        status: "",
        site_type: "2" // Filter for existing sites only
      };
      ...
      if (response.data?.data) {
        ...
        if (decryptedResponse.status === 200) {
          ...
          setError(decryptedResponse.message || "Failed to fetch existing site data");
          setAllData([]);
          setShowSites([]);
        }
      } else {
        setError("Failed to fetch existing site data");
        setAllData([]);
        setShowSites([]);
      }
    } catch (err) {
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      // 5️⃣ API call to update existing site
      ...
      if (decrypted.status === 200) {
        alert("Existing site updated successfully ✅");
        handleRefresh();           // reload list
        handleEditDialogClose();   // close modal
      } else {
        alert(decrypted.message || "Failed to update existing site");
      }

    } catch (error) {
      console.error("Update Existing Site Error:", error);
      alert("Something went wrong while updating existing site");
    } finally {
      setUploading(false);
    }
  };

  // Stats configuration for existing sites
  const statsCards = [
    {
      label: "Total Existing Sites",
      ...
      <Box sx={{ p: 2 }}>
        ...
        <Box sx={{
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <IconButton
              ...
              sx={{
                bgcolor: showFilters ? 'success.main' : 'grey.100',
                color: showFilters ? 'white' : 'grey.700',
                '&:hover': {
                  bgcolor: showFilters ? 'success.dark' : 'grey.200'
                }
              }}
            >
              <Tune fontSize="small" />
        ...
        <Box sx={{
          ...
          <Typography variant="body2" sx={{ color: 'grey.600', fontWeight: 600 }}>
            {`Showing ${showSites.length} existing sites${hasActiveFilters ? ' (filtered)' : ''}${loading ? ' (Loading...)' : ''}`}
          </Typography>
        ...
        >
          <DialogTitle>
            <Typography variant="h6" fontWeight={600}>
              Edit Existing Site: {selectedSite?.site_name || ''}
            </Typography>
          </DialogTitle>
          <DialogContent>
            {selectedSite && (
              <Grid container spacing={3} sx={{ mt: 1 }}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    multiline
                    rows={4}
                    placeholder="Add detailed information about the existing site..."
                  />
                </Grid>
...
export default withAuth(ExistingSitesReport);